      ...
      <div class="card mb-md flex flex-wrap">
        ...
        <div class="flex-grow px-md">
          ...
          <div class="flex flex-column gap-sm">
            ...
            <div class="flex align-items-center">
              <label style="width: 70px;">Luck</label>
              <input type="number" class="field field-visible text-center" style="width: 60px;" :value="saves.luck" @input="updateNestedField('saves', 'luck', parseInt($event.target.value))" :readonly="readOnly">
            </div>
          </div>
        </div>
        ...
        <div class="flex-grow pl-md">
          ...
          <div class="flex flex-column gap-sm">
            ...
            <div class="flex align-items-center">
              <label class="bold" style="width: 140px;">Effort</label>
              <div class="flex align-items-center">
                <input type="number" class="field field-visible text-center" style="width: 50px;" :value="currentEffort" @input="updateField('currentEffort', parseInt($event.target.value))" :readonly="readOnly">
                <span class="mx-xs">/</span>
                <input type="number" class="field field-visible text-center" style="width: 50px;" :value="maxEffort" @input="updateField('maxEffort', parseInt($event.target.value))" :readonly="readOnly">
              </div>
            </div>
            <div class="flex align-items-center">
              ...
              <div class="flex align-items-center">
                ...
                <span class="mx-xs">/</span>
                <input type="number" class="field field-visible text-center" style="width: 50px;" :value="maxSystemStrain" @input="updateField('maxSystemStrain', parseInt($event.target.value))" :readonly="readOnly">
              </div>
            </div>
          </div>
...
<h3 class="mb-sm mt-lg">Combat</h3>
      ...
      <div class="card mt-md">
          ...
          <table class="w-full weapon-table" v-if="weapons.length > 0">
            ...
            <tbody>
              <tr v-for="(w, i) in weapons" :key="i">
                ...
                <td><input type="text" class="field field-visible size-full text-center" :value="w.bonusToHit" @input="updateNestedField('weapons', i, { ...w, bonusToHit: $event.target.value })" :readonly="readOnly"></td>
                ...
                <td><input type="text" class="field field-visible size-full text-center" :value="w.range" @input="updateNestedField('weapons', i, { ...w, range: $event.target.value })" :readonly="readOnly"></td>
                <td><input type="text" class="field field-visible size-full" :value="w.specialShock" @input="updateNestedField('weapons', i, { ...w, specialShock: $event.target.value })" :readonly="readOnly"></td>
        ...
        <div class="table-wrapper">
          <table class="w-full armor-table" v-if="armor && armor.length > 0">
            <thead>
              <tr>
    ...
    <div class="page" v-show="view === 'abilities'">
      <text-section title="Foci" field="foci" :readonly="readOnly"></text-section>
      <text-section title="Magical Arts" field="magicalArts" :readonly="readOnly"></text-section>
      <text-section title="High Magic" field="highMagic" :readonly="readOnly"></text-section>
    </div>
      ...
      <div class="card mb-md">
        <div class="flex justify-content-between align-items-center mb-sm">
          ...
          <div class="flex align-items-center gap-md">
             <div><label class="mr-xs">GP:</label><input type="number" class="field field-visible text-right" style="width: 80px;" :value="wealth.gp" @input="updateNestedField('wealth', 'gp', parseInt($event.target.value))" :readonly="readOnly"></div>
             <div><label class="mr-xs">SP:</label><input type="number" class="field field-visible text-right" style="width: 80px;" :value="wealth.sp" @input="updateNestedField('wealth', 'sp', parseInt($event.target.value))" :readonly="readOnly"></div>
             <div><label class="mr-xs">CP:</label><input type="number" class="field field-visible text-right" style="width: 80px;" :value="wealth.cp" @input="updateNestedField('wealth', 'cp', parseInt($event.target.value))" :readonly="readOnly"></div>
          </div>
        </div>
        <div class="mt-sm">
           <label class="block mb-xs">Other Wealth:</label>
           <input type="text" class="field field-visible size-full" :value="wealth.other" @input="updateNestedField('wealth', 'other', $event.target.value)" :readonly="readOnly">
        </div>
      </div>

      <text-section title="Readied Items" field="readiedItems" :readonly="readOnly"></text-section>
      <text-section title="Stowed Items (Backpack)" field="stowedItems" :readonly="readOnly"></text-section>
      <text-section title="Stored Items" field="storedItems" :readonly="readOnly"></text-section>
      <text-section title="Ammunition" field="ammunitionText" :readonly="readOnly"></text-section>
    </div>

    <div class="page" v-show="view === 'details'">
      <div class="grid-layout">
        <div class="grid-col">
          <text-section title="Background Details" field="backgroundText" :readonly="readOnly"></text-section>
          <text-section title="Class Benefits" field="classBenefitsText" :readonly="readOnly"></text-section>
          <text-section title="Projects & Renown" field="projects" :readonly="readOnly"></text-section>
        </div>
        <div class="grid-col">
          <text-section title="Allies & Enemies" field="alliesAndEnemies" :readonly="readOnly"></text-section>
          <text-section title="Languages" field="languages" :readonly="readOnly"></text-section>
          <text-section title="Current Goals" field="goalsText" :readonly="readOnly"></text-section>
        </div>
      </div>
      <text-section title="Mount Details" field="mountDetails" :readonly="readOnly"></text-section>
      <text-section title="Description" field="descriptionText" :readonly="readOnly"></text-section>
    </div>

    <div class="page" v-show="view === 'notes'">
      <text-section title="Notes" field="notesText" :readonly="readOnly"></text-section>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState([
      'readOnly', 'characterName', 'class', 'level', 'xp', 'homeland', 'occupation', 'species', 'goal',
      ...
      'saves', 'skills', 'currentEffort', 'maxEffort', 'wealth', 'weapons', 'armor'
    ]),
  ...
  methods: {
    ...
    addWeapon() {
      this.$store.commit('addListItem', { list: 'weapons', item: { name: '', bonusToHit: '', damage: '', range: '', specialShock: '' } });
    },
    addArmor() {
