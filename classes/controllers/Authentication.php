...
class Authentication
{
    ...
    public function __construct($f3)
    {
        $this->f3 = $f3;
        $this->db = $f3->get('DB');
        $this->session = new \DB\SQL\Session($this->db, 'sessions', true, function ($sess) {
            return true; });
    }
    ...
    public function verify_ajax_csrf()
    {
        ...
        if (!$csrf)
            return false;
        if ($csrf !== $this->f3->get('SESSION.csrf'))
            return false;
        return true;
    }
    ...
    public function email_token($user, $url_path, $subject, $message)
    {
        ...
        $to = $email;
        $from = ['email' => 'unraid@benbrown-cs.com', 'name' => 'Minimal Character Sheet'];
        ...
        try {
            $result = $apiInstance->sendTransacEmail(sendSmtpEmail: $sendSmtpEmail);
        } catch (\Throwable $t) {
            error_log('Exception when sending email via Brevo: ' . $t->getMessage());
            if (method_exists($t, 'getResponseBody')) {
                error_log(print_r($t->getResponseBody(), true));
            }
        }
    }
